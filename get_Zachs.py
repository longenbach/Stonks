...
class Zachs():
    ...
    def get_earnings(self, ticker:str, start, end = datetime.date.today(), pandas = True, delay = 1):
        ...
        if not isinstance(start, datetime.date) or not isinstance(end, datetime.date):
            raise ValueError('Type error occurred with start or end parameters. Please enter valid date string or datetime object.')
    ...
    def __clean_vals(self, value:str):
        ...
        if value == '--':
            return np.nan
        elif value == 'N/A': # for Dates w/ no PEG
            return np.nan
        else:
    ...
    def __get_eps(self, browser, start, end, url, delay = 1):
        ...
        while done == False:
            ...
            # next_btn = browser.find_element_by_xpath('//*[@id="earnings_announcements_earnings_table_next"]') # deprecated
            next_btn = browser.find_element(By.XPATH,'//*[@id="earnings_announcements_earnings_table_next"]')
    ...
    def __get_revenue(self, browser, start, end, url, delay = 1):
        ...
        #sales_btn = browser.find_element_by_xpath('//*[@id="ui-id-4"]') # deprecated
        sales_btn = browser.find_element(By.XPATH,'//*[@id="ui-id-4"]')
        ...
        while done == False:
            ...
            # Find next page button
            # next_btn = browser.find_element_by_xpath('//*[@id="earnings_announcements_sales_table_next"]') # deprecated
            next_btn = browser.find_element(By.XPATH,'//*[@id="earnings_announcements_sales_table_next"]')
            location = next_btn.location
    ...
    def get_peg(self, ticker:str, start, end = datetime.date.today(), pandas = True, delay = 1):
        ...
        # Open zacks.com
        url = "https://www.zacks.com/stock/chart/" + ticker + "/fundamental/peg-ratio-ttm"
        browser.get(url)

         # passess same browser into get_revenue, returns revenue table
        pegs = self.__get_peg(browser, start, end, url, delay)
        ...
        # Depending on params, return dataframe or dictionary
        if pandas == True:
            return pd.DataFrame.from_dict(pegs, orient='index').sort_index(ascending=False)
        else:
            return pegs
    
    def __get_peg(self, browser, start, end, url, delay = 1):
        """
        Performs operations to scrape PEG data from www.zacks.com/stock/chart/{ticker}/fundamental/peg-ratio-ttm
        """
        ...
        # This loop iterates through each row, pulls data from each column then moves onto the next page to continue if neccessary
        # done represents whether we have reached the date in PEG table that we want to stop scraping
        while done == False:
            html = browser.page_source
            soup = BeautifulSoup(html, 'html.parser')

            # Try to find PEG table
            # If it cannot be found then the ticker probably is not reported on by zacks.com and we are looking at a searching results screen
            try:
                table = soup.find_all('table', id='DataTables_Table_0')[0].find('tbody')
                #print(table)
            except IndexError:
                error = "Encountered an error trying to access "+url+". The ticker you specified may not be available on Zacks.com."
                raise KeyError(error) from None
            ...
            # Create a list of all rows present on the screen
            rows = table.find_all('tr', attrs={'role':'row'}) # Ex. <tr class="odd" role="row"><td class="sorting_1">08/31/2020</td><td>3.68</td></tr>
            if len(rows) == 0: # If HTML doesn't have role="row", try find_all 'tr' no filter  
                rows = table.find_all('tr') # Ex. <tr class="odd"><td class="sorting_1" scope="row">01/31/2021</td><td>3.108</td></tr>
            #print(rows)
            #print("---")

            # Iterate through each row of PEG table and pull date from each column
            for row in rows:
                #print(row)
                # Pull results from each column
                col = row.find_all('td')
                ...
                # If we've reached a row that represents an earnings call that is withing our specified dates of inquiry then clean values and store them in return list
                # Otherwise skip this row or, if we have gone beyond latest date we want to pull then terminate loop and return results
                if date < start:
                    done = True
                    break

                elif date < end:
                    stats_list['PEG'] = self.__clean_vals(col[1].get_text()) # PEG Values
                    #stats_list['Ticker'] = ticker
                    #print(stats_list)
                    return_list[date] = stats_list
                stats_list = {}
            ...
            # Find next page button
            next_btn = browser.find_element(By.XPATH,'//*[@id="DataTables_Table_0_next"]')
            location = next_btn.location

            # If no more pages, stop
            tabindex = soup.find_all('a', id='DataTables_Table_0_next')[0]['tabindex'] # Ex. <a aria-controls="DataTables_Table_0" class="paginate_button next" data-dt-idx="7" id="DataTables_Table_0_next" role="button" tabindex="0">Next</a>
            if float(tabindex) == -1:
                done = True
                break
